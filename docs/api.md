...
> 状态：**冻结草案**。本契约是后端、CLI/SDK、DSH/Pi 插件的并行开发依据。
...
| 409 | `conflict` | 资源状态冲突（保留码，当前评分/上传均为幂等合并，不使用） |
| 413 | `too_large` | 请求体超限 |
...
目标：客户端本地已有一份目录快照时，只拉取差异，**避免每次全量下载浪费 2M 带宽**。
...
- 服务端另设**全局出站带宽看门狗**，接近 2M 阈值时优先对 `delta`/`list` 降级限流，保障 `get`/`random` 核心体验。
