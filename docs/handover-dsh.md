...
> **2026-09-03 实现注记：五条全部达成**，逐条证据在 `make smoke-dsh`（19/19）
...
> 记住 Pi 侧踩过的坑：把错误包成"正常返回一个带 error 字段的对象"会被当成成功，
...
而 pi 的 slash command 默认是把文本塞进上下文让模型动作（我们用 `setEditorText`
把正文填进输入框）。
...
我们的做法是**保持现有最小 frontmatter（只有 name + description），一份文件两边通用**。
...
1. **`argv` 不经 shell** → 提示词正文含引号/换行/`$` 都安全。Pi 侧我担心的注入
...
插件**必须**始终带 `--json`。Pi 侧我最初探测版本用 `bench --json`，结果 bench 把
...
> 私有函数**，不在 `bench-core.ts` 里 —— 所以得从入口文件拄代码，而不是 import。
...
| 本文原本写的 | 实测事实 | 影响 |
|---|---|---|
| 骨架里 `tag: { type: 'string', **required: false** }` | **可选属性必须省掉 `required` 键**；写 `required: false` 类型不过 | 直接拄骨架会编译失败，见 `plugins/dsh/index.ts` 真实写法 |
...
> **2026-09-03 实现注记：已交付。** `scripts/smoke-dsh.sh`（A 前置 → B 单元/tsc →
...
## 12. 未核实项与风险 → **已全部落地**（原清单保留供追溯）

以下我**没有**验证，别当作已知：

> **2026-09-03 实现注记：A–H 已全部核实/落地**，逐条结论见
> `benchmark-prompts/plugins/dsh/README.md` 的"本机实测核实记录"表与
> `scripts/smoke-dsh.sh`（其中 D/E/G 由源码直读 + 实跑双重确认；
> 表内"怎么验"列的方法均执行过）。下表保留原文供追溯。

| ID | 待核实 | 为什么重要 | 怎么验 |
|---|---|---|---|
| **A** | 本地未发布 npm 包的 `name` 解析方式（相对 profile 根？必须 npm 安装？） | 决定开发闭环是"改文件重启"还是"每次 pack/install" | 写一个只打印日志的最小插件，两种写法各试一次 |
| **B** | `ctx.subprocess` 的 `env` 净化是否真的会剥掉 `BENCH_*` | 用户靠环境变量配置时会神秘失效 | `spec.env` 留空跑一次 `bench config show --json`，看 endpoint 是否还是用户期望的 |
| **C** | 第三方工具是否受 `dsh-user-approval` / `dsh-authorization` / `dsh-permission-presets` 门控 | 若需每次批准，交互式测试体验会崩 | 注册一个工具，真实会话里调用，观察是否弹审批 |
| **D** | `presentCall`/`presentResult` 的 `card` 取值全集（`ToolCallView` 定义在 `dsh-tools/src/presentation.ts`） | 用错 card 名可能渲染空白 | 直接读该文件（有 src，能读） |
| **E** | DSH 里"把长文本交给用户复制/继续编辑"的对应原语 | Pi 侧靠 `setEditorText`；DSH 若无等价物，命令体验要重新设计 | 看 `dsh-client-ui-commands` / `dsh-client-ui-input-trigger` / `dsh-client-ui-conversation` 的 src |
| **F** | 只有 `web` profile 装在本机；**无头/CLI profile 的形态未验证** | 若你要支持 `dsh-headless`，命令与 UI 路径可能不同 | 读 `dsh-headless/src/index.ts` |
| **G** | DSH 的 TS 版本/模块解析（扩展名 `.ts` 导入是否允许，需不需要构建） | `dsh-tool-todo` 里 `import … from './types.ts'` 带 `.ts` 后缀，说明是 ESM/erasable-syntax 风格 | 用最小插件跑构建，看 loader 是否接受 TS 源 |
| **H** | **`docs/client.md` §11 缺 `canceled` / `local_error` 两个错误码** | 契约文档不完整，照文档实现会漏分支 | 已知事实，直接补文档（按 `internal/cli/cli.go` 为准） |
...
> **最后一条提醒**：本项目 M1 阶段的设计文档里，我曾凭经验臆造过一个 DSH/Pi 的
